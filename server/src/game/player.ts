...
export abstract class Player {
  ...
  public removeFromScore(value: number) {
    this._score.roundScore -= value;
    this._score.totalScore -= value;
  }
...
export class OnlinePlayer extends Player {
  ...
  constructor(name: string, userId: string, position: number, socket: SocketIO.Socket) {
    super(name, userId, position);
    this.socket = socket;
  }

  public sendUpdatedState(gameState: GameState) {
    if (this.socket == null) {
  ...
  public updateSocket(socket: SocketIO.Socket | null) {
    this.socket = socket;
  }
  ...
  public toBot(botGenerator: BotGenerator): Bot {
    let bot = botGenerator.generateRandomBot(this.userId, this.position);
